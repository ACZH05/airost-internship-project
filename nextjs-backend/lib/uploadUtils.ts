...
export async function uploadGroupPicture(file: File, groupId: string): Promise<string> {
    if (!file) {
        throw new Error("No file provided");
    }
    ...
    // Get the old group picture URL
    const group = await groupRef.get();
    const oldPictureUrl = group.data()?.profileUrl;
    ...
    // Create new file with renamed filename
    const fileExtension = file.name.split('.').pop();
    const newFileName = `pfp_${groupId}.${fileExtension}`;
    const renamedFile = new File([file], newFileName, { type: file.type });
